...
export class YearMonthSelector extends HTMLElement {
    ...
    #supportedModelPropertyNames = distinctList( [
    ...
    static get observedAttributes () {
        return [ // todo: run a pass, to see which may no longer be used
            'year', 'month', 'year-month', 'min-date', 'max-date'
        ];
    }
    ...
    connectedCallback () {
        // console.debug( '[YearMonthSelector]::connected' );
    }

    disconnectedCallback () {
        // console.debug( '[YearMonthSelector]::disconnected' );
    }

    adoptedCallback () {
        // console.debug( '[YearMonthSelector]::adopted' );
    }
    ...
    */
    ...
    updateChartFromSelf ( event ) {
        ...
        * alias as this.updateModel()
        ...
        if( ! event ) {
            console.log('no event, so using current state')
            chart.updateModel({
                year: state.year,
                month: state.month,
    ...
    rebuildAll () {
        ...
        style.textContent = `
        .yearMonthSelectorRoot, .yearMonthSelectorRoot.enableDebug {
        ...
        if ( this.verboseLogging ) {
            `[YearMonthSelector] :: rebuildAll :: yearMonthParam => new constraint = { min:  ${ minDate ?? NamedCodepoints.NullSymbol }, max: ${ maxDate ?? NamedCodepoints.NullSymbol
                } }`
        }
